    ...
    if task_name == 'drug-dose-prediction':
        ...
        for idx, (_, row) in enumerate(tqdm(df.iterrows(), total=len(df), desc="Drug dose prediction")):
            ...
            detailed_results.append({
                ...
                "raw_llm_output": output_text,
                "parsed_prediction": {
                    "max_dose": max_val if ok else None,
                    "min_dose": min_val if ok else None,
                    "avg_dose": avg_val if ok else None,
                },
                "ground_truth": {
                ...
                "correct": {
                    "max_dose": max_val == y_max[idx] if ok else False,
                    "min_dose": min_val == y_min[idx] if ok else False,
                    "avg_dose": avg_val == y_avg[idx] if ok else False,
                    "all": (max_val == y_max[idx] and min_val == y_min[idx] and avg_val == y_avg[idx]) if ok else False,
                },
            })
