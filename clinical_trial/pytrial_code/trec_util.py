...
def format_criteria_for_pytrial(criteria_text):
    if not criteria_text or not isinstance(criteria_text, str):
        return "inclusion\nNo criteria specified.\nexclusion\nNo criteria specified."
    
    # Check if inclusion/exclusion markers already exist
    has_inclusion = "inclusion" in criteria_text.lower()
    has_exclusion = "exclusion" in criteria_text.lower()
    
    if has_inclusion and has_exclusion:
        # Both markers exist, ensure they're on separate lines
        text = criteria_text.lower()
        inc_pos = text.find("inclusion")
        exc_pos = text.find("exclusion")
        
        # Ensure inclusion comes before exclusion
        if inc_pos < exc_pos:
            inc_part = criteria_text[inc_pos:]
            exc_part = criteria_text[exc_pos:inc_pos]
            return "inclusion\n" + inc_part + "\nexclusion\n" + exc_part
        else:
            # Reorder the text
            inc_part = criteria_text[inc_pos:]
        ...
        # Only inclusion exists, add exclusion
        return "inclusion\n" + criteria_text + "\nexclusion\nNone specified."
    
    elif has_exclusion:
        # Only exclusion exists, add inclusion before it
        return "inclusion\nNone specified.\n" + criteria_text
...
def load_trec_data(data_dir, selected_fields=None):
    ...
    try:
        ...
        print(f"Error loading TREC data: {e}")
        return None
...
if __name__ == "__main__":
    ...
    df.to_csv("trec_data_2.csv", index=False)